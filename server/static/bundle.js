...
(function() {
  ...
  init = function() {
    login_view.setup();
    sockets.setup();
    return console.log('app launched.');
  };
...
},{"./lib/login_view.coffee":3,"./lib/sockets.coffee":4,"jquery":5}],5:[function(require,module,exports){
(function(){/*!
...
(function() {
  var Bacon, io, isSignalFresh, isSignalGood, isTruthy, moment, timediff;
  ...
  timediff = function(earlier, later) {
    return moment(later).diff(earlier, 's');
  };

  isSignalFresh = function(data, now) {
    if (timediff(new Date(data.reading_time), now) < 20) {
      return true;
    } else {
      return false;
    }
  ...
  exports.setup = function() {
    var isSignalGoodProp, mindwaveData, signalFreshness, socket;
    mindwaveData = null;
    socket = io.connect('http://localhost:5000');
    socket.emit('connected', 'sup');
    socket.on('server_says', function(data) {
      return console.log('server says: ', data);
    });
    socket.on('data', function(data) {
      mindwaveData = data;
      return console.log('mw data: ', data);
    });
    isSignalGoodProp = Bacon.fromEventTarget(socket, 'data').map(isSignalGood).toProperty(false);
    return signalFreshness = Bacon.interval(1000).filter(function() {
      return isTruthy(mindwaveData);
    }).map(function() {
      return isSignalFresh(mindwaveData, new Date());
    }).log('signal is fresh?');
  };
...
},{"./socket.io.min.js":1,"baconjs":6,"moment":7}],6:[function(require,module,exports){
...
},{"jquery":5,"lodash":8}],8:[function(require,module,exports){
(function(global){/**
 * @license
